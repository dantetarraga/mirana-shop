...
export default function ProductsPage() {
  ...
  const columns = useMemo<Column<Product>[]>(() => [
    ...
    { header: "SKU",       className: A.mono,    render: (p) => p.sku },
    { header: "Categoría", render: (p) => CAT_LABELS[p.cat] },
    { header: "Precio",    className: A.valGold, render: (p) => `$${p.price.toFixed(2)}` },
      ...
      {editing && (
        ...
        >
          <FormField label="Nombre">
            <input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className={A.input} />
          </FormField>
          <div className="grid grid-cols-2 gap-3.5">
            <FormField label="SKU">
              <input value={editing.sku} onChange={(e) => setEditing({ ...editing, sku: e.target.value })} className={A.input} />
            </FormField>
            <FormField label="Categoría">
              <select value={editing.cat} onChange={(e) => setEditing({ ...editing, cat: e.target.value as Product["cat"] })} className={A.input}>
            ...
            <FormField label="Precio">
              <input type="number" step="0.01" value={editing.price} onChange={(e) => setEditing({ ...editing, price: parseFloat(e.target.value) || 0 })} className={A.input} />
            </FormField>
            <FormField label="Stock">
              <input type="number" value={editing.stock} onChange={(e) => setEditing({ ...editing, stock: parseInt(e.target.value) || 0 })} className={A.input} />
            </FormField>
          </div>
          <FormField label="Badge (opcional)">
            <input placeholder="NUEVO, BESTSELLER..." value={editing.badge || ""} onChange={(e) => setEditing({ ...editing, badge: e.target.value || null })} className={A.input} />
          </FormField>
          <FormField label="Descripción">
            <textarea rows={3} value={editing.desc} onChange={(e) => setEditing({ ...editing, desc: e.target.value })} className={cn(A.input, "resize-y")} />
          </FormField>
