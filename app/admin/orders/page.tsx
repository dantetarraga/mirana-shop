          ...
          <DrawerSection title="Artículos">
            {detail.items.map((it, i) => (
              <div key={i} className="flex justify-between py-2">
                ...
                <span className="font-display font-extrabold text-[15px] text-(--gold)">
                  ${fmt(it.price * it.qty)}
                </span>
              </div>
            ))}
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(ORDER_STATUS).map(([k, v]) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setOrderStatus(detail.id, k)
                    setDetail({ ...detail, status: k as Order['status'] })
                  }}
