...
export function AuthModal() {
  ...
  const I = ({ label }: { label: string }) => (
    <label className="text-[10px] tracking-[2px] uppercase text-muted mb-[6px] block">{label}</label>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="font-display text-[32px] font-black uppercase tracking-[-0.5px] leading-none mb-[6px]">
        ...
        <form onSubmit={submit}>
          {mode === "register" && (
            <div className="mb-[14px]">
              ...
              <input className="adm-input" placeholder="Tu nombre" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
          )}
          <div className="mb-[14px]">
            <I label="Correo electrónico" />
            <input type="email" className="adm-input" placeholder="tucorreo@ejemplo.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </div>
          <div className="mb-[14px]">
            <I label="Contraseña" />
            <input type="password" className="adm-input" placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
          </div>
          {mode === "register" && (
            <div className="mb-[14px]">
              <I label="Confirmar contraseña" />
              <input type="password" className="adm-input" placeholder="••••••••" value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} />
            </div>
          )}
          {err && <div className="text-[#ff6644] text-[12px] mb-[10px]">{err}</div>}
          <div className="flex justify-between items-center text-[12px] text-muted my-[6px] mb-[18px]">
            {mode === "login" ? (
              <><label className="flex items-center gap-[6px] cursor-pointer"><input type="checkbox" /> Recordarme</label>
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="md" onClick={() => authenticate({ name: "Usuario Google", email: "demo@gmail.com" })} className="flex-1">
          ...
          <Button variant="outline" size="md" onClick={() => authenticate({ name: "Admin Mirana", email: "admin@mirana.com" })} className="flex-1">
            <LayoutGrid size={14} />
            Demo Admin
          </Button>
        </div>
