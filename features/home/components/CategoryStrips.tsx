  ...
  return (
    <section className="px-12 pb-20 grid grid-cols-3 gap-4">
      {featured.map((cat) => (
        ...
        >
          <div
            className={`${STRIPE_MAP[cat.slug] ?? "stripe-fig"} h-[180px] flex items-end px-6 pb-6 relative border border-(--bd) transition-[border-color] duration-[.25s]`}
          >
            <div>
              ...
              {cat.description && (
                <div className="text-[12px] text-muted mt-1 line-clamp-1">
                  {cat.description}
                </div>
              )}
            </div>
