...
export function PreorderSection() {
  return (
    <section className="px-12 py-20">
      <div className="mb-8">
        <div className="text-[10px] font-bold tracking-[3px] uppercase text-(--gold) mb-2.5">Disponibles pronto</div>
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
        {[
          ...
          <div key={p.name} className="bg-card border border-(--bd) cursor-pointer">
            <div className={`${p.cat} h-[200px] relative flex items-center justify-center`}>
              <div className="absolute top-3 left-0 bg-(--gold) text-black text-[9px] font-extrabold tracking-[2px] uppercase px-2.5 py-[5px]">{p.badge}</div>
            ...
            <div className="p-5">
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="font-display text-[24px] font-black text-(--gold)">${p.price.toFixed(2)}</div>
                <div className="text-[11px] text-muted border border-(--bd) px-2.5 py-1 tracking-[1px] uppercase">
                  Llegada {p.arrival}
                </div>
              </div>
