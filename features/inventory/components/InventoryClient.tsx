...
import type { StockFilter } from "@/modules/catalog/repositories/product.repo";
...
export function InventoryClient({ initialProducts, initialStats, currentFilter }: InventoryClientProps) {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setProducts((prev) => prev.map((x) =>
          x.id === p.id ? { ...x, inventory: { availableStock: result.data.newStock } } : x
        ));
      ...
      render: (p) => {
        ...
        return (
          <div className="flex gap-1.5 items-center">
            <Button variant="icon" size="sm" onClick={() => adjust(p, Math.max(0, stock - 1))} disabled={isPending}><Minus size={14} /></Button>
            <input
              ...
              disabled={isPending}
            />
            <Button variant="icon" size="sm" onClick={() => adjust(p, stock + 1)} disabled={isPending}><Plus size={14} /></Button>
            <Button variant="ghost" size="sm" onClick={() => adjust(p, stock + 50)} className="ml-2" disabled={isPending}>+50</Button>
          </div>
        );
      },
    },
  ...
  return (
    <div className="px-8 pt-7 pb-12">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard label="Valor inventario" value={`S/ ${(initialStats.totalValue / 1000).toFixed(1)}K`}           valueClass="text-(--gold)" />
        <KpiCard label="Stock bajo"       value={initialStats.lowStockCount}                                      valueClass="text-[#ffb84a]" />
        <KpiCard label="Agotados"         value={initialStats.outOfStockCount}                                    valueClass="text-[#ff6644]" />
      </div>
      ...
      <div className="flex gap-1.5 mb-5">
        {FILTER_TABS.map(({ key, label }) => {
          ...
          return (
            <a key={key} href={key === "all" ? "/admin/inventory" : `/admin/inventory?filter=${key}`}
              className={cn(
            ...
            >
              {label} <span className="opacity-70 ml-1 font-sans normal-case tracking-normal text-[12px]">{products.length}</span>
            </a>
          );
        })}
      </div>
