...
export const orderRepo = {
  ...
  // ---------------------------------------------------------------------------
  // updateStatus — el admin acepta/cancela un pedido tras validar el comprobante
  //
  // Al aceptar (-> PAID) el stock reservado se da por vendido (finalizeReservedStock).
  // Al cancelar (-> CANCELLED/REFUNDED) el stock reservado vuelve a disponible
  // (releaseReservedStock). Todo ocurre en una sola transacción con el update.
  // ---------------------------------------------------------------------------
  async updateStatus(
    id: string,
    status: OrderStatus,
    ...
    return db.$transaction(async (tx) => {
      ...
      if (isCancelling) {
        for (const item of current.items) {
          await inventoryRepo.releaseReservedStock(tx, {
      ...
      if (isAccepting) {
        for (const item of current.items) {
          await inventoryRepo.finalizeReservedStock(tx, {
            productId: item.productId,
            quantity: item.quantity,
            orderId: id,
          })
  ...
  //
  // Flujo manual por WhatsApp: la orden nace "Pendiente de comprobante"
  // (AWAITING_PROOF) y se reserva el stock de cada item. Si el stock no
  // alcanza para algún producto, toda la transacción (orden + reserva) se
  // revierte y no queda nada creado.
  // ---------------------------------------------------------------------------
  async create(input: CreateOrderInput): Promise<{ id: string; code: string }> {
    return db.$transaction(async (tx) => {
      ...
      for (const item of input.items) {
        await inventoryRepo.reserveStockForOrder(tx, {
          productId: item.productId,
          quantity: item.quantity,
          orderId: order.id,
