...
function getCategoryStripe(slug: string): string {
  const map: Record<string, string> = {
    "figuras-accion": "stripe-fig", lego: "stripe-lego",
    "modelos-escala": "stripe-veh", anime: "stripe-fig",
  };
  return map[slug] ?? "stripe-fig";
}

const STATUS_LABELS: Record<string, string> = {
  AVAILABLE: "Disponible", PREORDER: "Preventa", SOLD_OUT: "Agotado",
  COMING_SOON: "Próximamente", ARCHIVED: "Archivado",
};
...
const EMPTY_FORM: ProductFormValues = {
  name: "", slug: "", sku: "", description: "", price: 0, stock: 0,
  categoryId: "", brandId: "", status: "AVAILABLE", featured: false,
};
...
export function ProductsClient({
  ...
  total, currentPage, perPage, currentQ, currentCat,
}: ProductsClientProps) {
  ...
  const openEdit = (p: SerializedProduct) => { setIsNew(false); setEditingId(p.id); };
  const openNew  = () => { setEditingId(null); setIsNew(true); };
  const closeDrawer = () => { setEditingId(null); setIsNew(false); reset(EMPTY_FORM); };

  const onSubmit = (data: ProductFormValues) => {
    startTransition(async () => {
      if (isNew) {
        const result = await createProduct(data);
        if (result.success) { toast.success("Producto creado"); closeDrawer(); window.location.reload(); }
        ...
        if (result.success) {
          ...
          setProducts((prev) => prev.map((p) => p.id === editingId
            ? { ...p, name: data.name, sku: data.sku, slug: data.slug, price: data.price ?? p.price,
                status: data.status ?? p.status, featured: data.featured ?? p.featured,
  ...
  const handleDelete = (p: SerializedProduct) => {
    if (!confirm(`¿Eliminar "${p.name}"?`)) return;
    startTransition(async () => {
      const result = await deleteProduct(p.id);
      if (result.success) { setProducts((prev) => prev.filter((x) => x.id !== p.id)); toast.success(`"${p.name}" eliminado`); }
      else toast.error(result.error);
    });
  };
    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex gap-1.5 justify-end">
          <Button variant="icon" size="sm" onClick={(e) => { e.stopPropagation(); openEdit(p); }}><Pencil size={14} /></Button>
          <Button variant="icon" size="sm" destructive onClick={(e) => { e.stopPropagation(); handleDelete(p); }}><X size={14} /></Button>
        </div>
      ),
    },
    ...
    <div className="px-8 pt-7 pb-12">
      ...
      <div className="flex items-center gap-3.5 flex-wrap mb-4">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {[{ key: "all", label: "Todos" }, ...categories.map((c) => ({ key: c.slug, label: c.name }))].map(({ key, label }) => {
            const isActive = key === currentCat;
            const href = buildUrl({ q: currentQ || undefined, cat: key !== "all" ? key : undefined });
            return (
              <a key={key} href={href}
                className={cn(
                  "px-3.5 py-2 text-[11px] tracking-[1px] uppercase font-display font-extrabold border transition-colors",
                  isActive ? "bg-(--gold) border-(--gold) text-black" : "border-(--bd) text-muted hover:text-text"
                )}
              >
                {label}
              </a>
      ...
      <AdminTable columns={columns} data={products} keyExtractor={(p) => p.id} onRowClick={openEdit} />

      {/* Paginación */}
      {totalPages > 1 && (
        <div className="flex gap-2 justify-end mt-4">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <a key={p} href={buildUrl({ q: currentQ || undefined, cat: currentCat !== "all" ? currentCat : undefined, page: String(p) })}
              className={cn(
      ...
      {showImport && <ExcelImportDrawer onClose={() => setShowImport(false)} onImport={handleImport} />}
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-4.5">
            ...
            <div className="flex gap-2.5">
              ...
              <Button type="button" variant="outline" size="md" full onClick={closeDrawer}>Cancelar</Button>
            </div>
          </form>
        </AdminDrawer>
      )}
